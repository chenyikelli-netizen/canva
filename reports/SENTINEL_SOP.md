...
> 月報為**手動觸發**的管理層級報告，以每月底或月中重要決策節點為準，不與每日自動流程衝突。

### 觸發時機

| 時機 | 說明 |
|------|------|
| 每月底（月最後一個工作日） | 彙整當月全部日報 |
| 重大事件後（如競品重大動作、品牌危機） | 即時月中特刊 |
| 決策會議前 | 依需求手動產出 |

### 月報的四層架構（必要結構）

每份月報必須包含以下四個層次，**順序固定、不可合併**：

| 層次 | 內容定義 | 輸出標準 |
|------|----------|----------|
| **Layer 1 月度定論** | 一句話總評 + 三個支撐 KPI | 正向均值、單日最高負向、Canva 聲量占比 |
| **Layer 2 三個關鍵判斷** | 每個判斷固定三行：事件→意味著→建議行動 | 必須涵蓋：品牌風險、溝通風險、商機切入口各一個 |
| **Layer 3 競爭格局位移** | 固定三品牌 × 四維度矩陣表 | 聲量占比、月末趨勢、威脅方向、本月最顯著動作 |
| **Layer 4 下月觀察錨點** | 2–3 個可驗證的條件句 | 格式：「若 _____ 發生，代表 _____ 惡化／好轉」 |

### 內容品質要求

> 月報中**每一行**必須可獨立回答「所以呢？」。禁止放入純描述性資料——所有輸入必須是結論句或判斷句，而非資料彙整。

**合格範例**：
> 月中負向急升至 44%，顯示品牌在學生族群中出現認知裂縫

**不合格範例**：
> 正向情緒：52.9%（缺乏解釋意義，屬於資料彙整）

### HTML 月報視覺規格

月報的 HTML 版本（`.html`）為主要發布格式，設計規格如下：

- **色彩系統語意**：紅色 = 需介入的風險｜黃色 = 需追蹤的警示｜綠色 = 可推進的機會
- **判斷卡片**：左側帶對應顏色發光色條，背景帶對應色調漸層（不得使用統一深灰）
- **KPI 數字**：字體 ≥ 28px，帶對應顏色 `text-shadow` 光暈效果
- **背景**：主背景帶角落極淡漸層光暈（非純色）
- **完全內聯 CSS**：不依賴任何外部 CDN（Google Fonts 除外）
- **Hover 效果**：判斷卡片 hover 時向上位移 2px + 加深陰影
- **參考模板**：`reports/2026-04-月報-canva-brand-sentinel.html`

### 月報命名規則

```
reports/YYYY-MM-月報-canva-brand-sentinel.md   ← Markdown 版（決策記錄）
reports/YYYY-MM-月報-canva-brand-sentinel.html ← HTML 版（主要閱讀格式）
```

### 月報驗收清單

```
□ Layer 1：一句話定論明確，三個 KPI 各有解釋意義（非純數字）
□ Layer 2：三個判斷各自對應不同性質（風險 / 溝通 / 商機）
□ Layer 2：每個判斷皆有「事件 → 意味著 → 行動」三行
□ Layer 3：競品表格四維度齊全，末行有格局小結
□ Layer 4：錨點為可驗證條件句，非預測或展望
□ HTML 視覺：判斷卡片有三色區分（紅/黃/綠）
□ HTML 視覺：KPI 數字有光暈效果，背景非純黑
□ 所有行文符合「結論句」標準，無純資料描述
□ 推送至 GitHub（save_git.bat）
```
...
│   ├── YYYY-MM-DD-canva-report.md           每日日報（Markdown）
│   ├── YYYY-MM-DD-canva-report-promax.html  每日日報（HTML Pro Max）
│   ├── YYYY-MM-月報-canva-brand-sentinel.md   月報（Markdown）
│   └── YYYY-MM-月報-canva-brand-sentinel.html 月報（HTML，主閱讀格式）
